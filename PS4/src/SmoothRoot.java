...
public class SmoothRoot {
	
	private ArrayList<SmoothLeafNode> children;
	private double[] p;
	private int totalData;
	private int totalTrue;
	
	public SmoothRoot(ArrayList<String> nodeNames, ArrayList<Integer> numeric) {
		this.children = new ArrayList<SmoothLeafNode>(nodeNames.size());
		for(int i = 0; i < nodeNames.size(); i++) {
			if(numeric.get(i) == 0) {
				children.add(new SmoothNumericLeaf(nodeNames.get(i), 0, this));
			} else {
				children.add(new SmoothCategoryLeaf(nodeNames.get(i), numeric.get(i), this));
			}
		}
		this.p = new double[2];
		this.totalData = 0;
	...
	public void calculateProbabilities() {
		...
		for(SmoothLeafNode child : this.children) {
